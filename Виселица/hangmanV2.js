function greeting(turnCount, answerArray) {
    document.getElementById("displayStatus").innerHTML = " Игра на угадывание слов. В этом варианте игры " +
    "первый игрок будет загадывать слово, а второй отгадывать его. <br>У Вас будет " + turnCount + " попыток.<br>" +
    "Загаданое игроком слово:<br>" + answerArray.join(" ") + "<br>Нажмите Далее, чтобы начать.";  // полностью описать правила
}
    ...
    if (remainingLetters == 0) {
        ...
        if (turnCount == 1) {
            ...
            if (correctLetter == false) {
                ...
                document.getElementById("displayStatus").innerHTML = "Введённая буква верна:<br>" + answerArray.join(" ")
                 +  "<br>У Вас осталось попыток: " + turnCount;
                 document.getElementById("showAlreadyUsedLetters").innerHTML = 'Использованные буквы:<br>' + alreadyUsedLetters.join(', ');;
        }
        }
...
function showAnswerAndRatePlayer(turnCount, word) {
    if (turnCount == 0) {
        document.getElementById("displayStatus").innerHTML = "Попытки кончились! Было загадано слово: \""
         + word + "\". Можно взять реванш :)";
    } else {
...
function exitGame(word) {
    document.getElementById("displayStatus").innerHTML = "Игра окончена. Было загадано слово: " + word + ".";
    document.getElementById("guessOfPlayer").style.display = 'none';
    document.getElementById("interactWithPlayer").style.display = 'none';
    document.getElementById("confirmActions").style.display = 'none';
    document.getElementById("exitButton").style.display = 'none';
